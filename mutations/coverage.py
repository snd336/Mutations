...
class AbstractCoverageNodeTransformer(ast.NodeTransformer):
    ...
    def __init__(self):
        ...
        for node_class in self.get_coverable_nodes():
            visit_method_name = 'visit_' + node_class.__name__  # visit_ast.Assign, the stmt
            if not hasattr(self, visit_method_name):            # where ast.BinOp is expr
                if node_class == ast.ExceptHandler:             # ast.Add is operator
    ...
    def generate_coverage_node(self, node):
        if hasattr(node, 'body'):
            ...
            markers = self.get_included_markers(node)                # __covered_nodes__.update(?)
