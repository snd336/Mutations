...
from src.mutpy.controller import *
from src.mutpy import operators, utils, codegen
from src.tests.mutpytests.utils import MockModulesLoader
from src.mutpy.test_runners import UnittestTestRunner
...
class MockMutationController(MutationController):
...
class MutationControllerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mutator = FirstOrderMutator([operators.ArithmeticOperatorReplacement], percentage=100)
...
class FirstToLastHOMStrategyTest(BaseHOMStrategyTest):
    def test_generate(self):
        ...
        changes_to_apply = self.apply_strategy_to_mutations_with_order_2(FirstToLastHOMStrategy, mutations)
...
class EachChoiceHOMStrategyTest(BaseHOMStrategyTest):
    def test_generate(self):
        ...
        changes_to_apply = self.apply_strategy_to_mutations_with_order_2(EachChoiceHOMStrategy, mutations)
...
class BetweenOperatorsHOMStrategyTest(BaseHOMStrategyTest):
    def test_generate_if_one_operator(self):
        ...
        changes_to_apply = self.apply_strategy_to_mutations_with_order_2(BetweenOperatorsHOMStrategy,
                                                                         mutations)
        self.assert_num_changesets(changes_to_apply, 2)
        self.assert_num_changeset_entries(changes_to_apply, 0, 1)
        self.assert_mutation_in_changeset_at_position_equals(changes_to_apply, 0, 0, mutations[0])
...
class RandomHOMStrategyTest(BaseHOMStrategyTest):
    def test_generate(self):
        ...
        changes_to_apply = self.apply_strategy_to_mutations_with_order_2(RandomHOMStrategy, mutations,
